...
- `.env.example` 当前默认把 `response / embedding / vision` bootstrap 收到本机 Ollama，并把 `INITIAL_OLLAMA_BASE_URL` 设为 `http://localhost:11434`
...
- Web 侧聊天运行时边界当前进一步收敛为“`useChatRuntimeState` 读 `streamRun`、`useChatRuntimeController` 管会话级提交锁和 `streamRun` action、`useChatSessionCacheActions` 统一 patch `messagesWindow / context` 与 started user message”；页面装配层不再自己直写 Query Cache
- Web 侧登录回跳当前改走 `/login?redirect=...` 这条 URL 契约；前端 session store 只保留登录态，不再存 `redirectTo`
