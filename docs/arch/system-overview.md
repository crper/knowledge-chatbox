...
- **质量工具**：本地继续保留 `ruff`（格式与静态规则）+ `basedpyright`（类型边界）+ `pytest`（行为回归）；当前 GitHub blocking CI 先收敛为 `ruff + pytest + web + repo-surface`
- **仓库自动化**：GitHub Actions 固定执行 `api / web / repo-surface` 三条检查；Dependabot 的 `patch / minor` PR 会在检查通过后自动审批并开启自动合并
