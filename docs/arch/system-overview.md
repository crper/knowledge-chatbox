...
- 日常本地开发走 `just dev`、`just api-dev`、`just web-dev`
- `just reset-dev` 先重置本地数据，再同步依赖并拉起同一套开发态脚本，适合把乱掉的本地状态快速拉回干净可运行态
...
| 领域 | 当前能力 |
| --- | --- |
| 认证与权限 | 基于 `PyJWT` 的短期 bearer `access token` + HttpOnly `refresh session`；`admin / user` 两类角色；用户级偏好与账号安全；前端启动期与接口 `401` 会自动刷新 access token；资源上传和 SSE 流式聊天也会先带 bearer token，必要时单飞刷新并重试一次；修改密码弹窗会对“当前密码缺失 / 新密码缺失 / 原始密码错误”展示语义化、可国际化提示 |
| 资源管理 | 上传、逻辑资源列表、修订历史、删除、重建索引、文件下载 |
| 文档处理 | `txt / md / pdf / docx / png / jpg / jpeg / webp` 标准化 |
| 问答 | 同步问答、流式问答、来源引用、失败重试、活动 run 查看、长会话虚拟消息视口 |
...
3. 标准化阶段读取 vision route
4. 索引阶段读取 embedding route
5. 成功后把 latest revision 指针移动到新修订
...
- 多文档附件会按附件逐个召回后再合并，避免单个文档吃满全局 `top_k`
...
- 图片附件在真正发给 response provider 前，会从已持久化的 `document_revision` 重新读取原图并统一转成稳定的 JPEG payload，避免 provider 因原始格式差异或文件名异常直接拒收
- Chroma 查询优先使用 query embedding，召回不足时再退回本地轻量词法匹配；弱命中不会注入 prompt，也不会返回 `sources_json`
...
- SQLite 连接层默认开启 `WAL` 和 `busy_timeout=5000`，进一步降低流式事件写入与标准读取并发时直接触发锁错误的概率
...
2. 看 [repo-map-and-conventions.md](./repo-map-and-conventions.md)
3. 看 [database-design.md](./database-design.md)
4. 看 [provider-and-settings.md](./provider-and-settings.md)
5. 再按需要进入具体实现文件
