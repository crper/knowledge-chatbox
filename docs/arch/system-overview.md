...
2. 上传内容先按块落到本地 `uploads` 目录，并同步计算 `content_hash + file_size`
3. 追加 `document_revisions`
4. 文本文档（`txt / md / pdf / docx`）在请求内完成标准化与索引
5. 图片文档（`png / jpg / jpeg / webp`）先返回 `processing`，随后由后台任务补做 vision 标准化与索引
6. 成功后把 latest revision 指针移动到新修订
...
- Chroma 查询优先使用 query embedding，召回不足时再退回本地轻量词法匹配；若 query embedding 在本轮生成阶段直接失败，则当前问答会继续走本地轻量词法匹配，而不是直接退化成无检索上下文回答；弱命中不会注入 prompt，也不会返回 `sources_json`
