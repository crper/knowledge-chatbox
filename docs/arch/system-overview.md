...
- 单机部署：`just docker-up`（启动前会按当前工作区重建镜像）
- 数据重置：`just reset-dev`

补充约束：

- `just init-env` 会自动补齐本地开发所需的 `JWT_SECRET_KEY` 和 `INITIAL_ADMIN_PASSWORD`
- `just dev` / `just reset-dev` 启动时会先预检 `API_PORT / WEB_PORT` 是否空闲；`just reset-dev` 只有在预检通过后才会清空本地数据
- `just dev` / `just reset-dev` 启动时会提示 bootstrap 管理员账号，并明确告诉你登录密码应回看 `.env`
- 后端 Python 基线当前固定在 `apps/api/.python-version` 的 `3.13.13`；本地、CI、Docker 都应围绕同一条 3.13 主线验证

### 收敛规则

- **设置真相源**：`AppSettings` 是持久化真相源；运行时统一消费强类型 `ProviderRuntimeSettings`，不再接受 dict-like 输入
- **设置审计**：`settings_versions` 额外记录脱敏设置快照与变更字段，当前用于追溯，不作为 secret 回滚真相源
- **聊天执行**：同步和流式问答统一由 `ChatWorkflow + PydanticAI` 驱动，共享 HTTP 契约、`sources_json` 和 `client_request_id` 幂等语义
- **服务装配**：认证和用户管理相关 service 通过 `core/service_builders.py` 做显式 composition，`api/deps.py` 与启动补偿链路共用同一套装配规则
- **质量门禁**：`ruff`（格式与静态规则）+ 分层 `basedpyright`（`src` 严格、`tests` 宽松）+ `pytest`（行为回归）；异步测试统一由 `pytest-asyncio` 的 `asyncio_mode=auto` 承接
...
- 登录限流基于 `rate_limit_attempts` 数据库表，确保多 worker 部署下限流一致；进程内短期内存缓存（1s TTL）减少 DB 查询
- `JWT_SECRET_KEY` 必须通过 `.env` 显式设置；`INITIAL_ADMIN_PASSWORD` 只在空库首次 bootstrap 管理员时需要，本地开发可由 `just init-env` 自动补齐
...
- 文件下载接口通过 `_safe_file_path` 做路径遍历防护：确保请求的文件路径在 `upload_dir` 或 `normalized_dir` 内
- 生产环境自动禁用 `/docs`、`/redoc`、`/openapi.json`
- CORS 不允许 `*` 通配；`allow_methods` 和 `allow_headers` 限定为已知值
- 内部异常对外统一返回脱敏消息，原始错误仅记录日志
...
- `settings_versions` 记录每次设置变更的脱敏快照与 changed fields
...
- 资源页 PDF 当前由 `EmbedPDF` 统一承接桌面主预览和移动端抽屉内嵌预览；受保护文件继续走同源 `/api` + cookie 鉴权读取
...
- 设置审计采用 `settings_versions` 独立承接，不把追溯逻辑再塞回 `app_settings`
...
- 前端聊天运行态当前收口为 `useChatRuntime + useChatCacheWriter + useChatComposerStore` 三层边界：运行态读写、Query cache 写出口、composer 本地状态各自单点负责
