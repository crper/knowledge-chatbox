...
4. 标准化阶段读取 vision route
5. 索引阶段读取 embedding route
...
- Chroma 查询优先使用 query embedding，召回不足时再退回本地轻量词法匹配；若 query embedding 在本轮生成阶段直接失败，则当前问答会显式降级为“无检索上下文回答”，不再退回整代索引的全量词法扫描；弱命中不会注入 prompt，也不会返回 `sources_json`
