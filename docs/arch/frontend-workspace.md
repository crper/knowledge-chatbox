...
- 弹层、菜单、抽屉、选择器、滚动区等基础交互优先复用 `Base UI`
- 组合方式优先走 `render`，不再新增 `asChild`
- `Button` 保持按钮语义，不再拿按钮组件直接渲染链接；需要“长得像按钮的链接”时，直接复用 `buttonVariants`

这样做的目标很简单：

- 基础可访问性和状态机尽量交给同一套底层能力
...
- 横向可滚动的设置分组入口
- 导航和上下文通过抽屉或局部展开承接
- 资源页把状态指标降成列表区内的紧凑摘要，不让主列表被大卡片后推
...
当前顶层路由与共享守卫语义定义在 `apps/web/src/routes/*`、`apps/web/src/router/bootstrap-gate.tsx`、`apps/web/src/router/route-shells.tsx`：
...
- `/chat` 会先在 route 层恢复最近可用会话；如果没有任何会话，才保留空入口态
- `/chat/:sessionId` 命中非法或失效会话时，会先在 route 层归一化回可用会话或 `/chat`
- 普通用户访问 `/users` 会看到 `403` 页面
- `/settings` 和旧 `?section=` 入口会先在 route 层归一化到 `/settings/:section`
- 越权或非法的 settings section 会在 route 层直接重定向到当前用户可访问的 canonical section
