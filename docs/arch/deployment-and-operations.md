...
- `INITIAL_ADMIN_PASSWORD` 只在空库首次 bootstrap 管理员时需要；如果当前实例已有管理员，缺失它不会再阻断启动
- 如果要 bootstrap 首个管理员，`INITIAL_ADMIN_PASSWORD` 仍必须满足复杂度要求：至少 8 字符，且包含大写字母、小写字母、数字、特殊字符中的至少 3 类
...
- 资源页 PDF 内嵌预览当前由 `EmbedPDF` 驱动，会直接对受保护文件 URL 发起 credentialed 请求；开发态若把 API 切到跨源地址，必须同时确认 cookie 与 CORS 仍允许这条读取链路
...
- SQLite 也会保存 `settings_versions` 脱敏审计快照；它用于追溯设置变更，不保存 secret 明文
...
- 构建上下文：仓库根目录 `.`，Dockerfile 位于 `apps/api/Dockerfile`
...
- 资源页 PDF 预览当前使用 `@embedpdf/react-pdf-viewer`；生产构建会额外产出对应静态资源，首次改动这条预览链路后应至少执行一次 `vp build` 或 `just docker-build` 确认产物完整
...
- `up` 会在启动前执行 `docker compose up --build`，避免继续复用旧镜像里的前端静态资源或后端代码
...
`just docker-up` 当前会始终按工作区重建镜像，然后再拉起服务。这样代价是启动时会多一次 build 检查，但好处是前端静态资源、后端代码和 lockfile 变更不会再悄悄吃旧镜像。`just docker-build` 仍保留给想单独预热镜像或只排查构建阶段时使用。
...
## 5. `scripts/reset-local-data.sh` 怎么用
...
ENV_FILE=/abs/path/.env ./scripts/reset-local-data.sh --yes
just setup
just reset-dev
```
...
| ------------------------ | ------------------------------------------------ | ------------------------------------------------- | ---------------------------------------------- |
| 首次本地 Docker 启动     | `just init-env && just docker-up`                | `cp .env.example .env && scripts/docker-deploy.sh up` | 首次启动或当前工作区刚有代码改动              |
| 只想单独预热镜像         | `just docker-build`                              | `scripts/docker-deploy.sh build`                  | 想把构建失败和运行失败拆开排查                 |
...
| 彻底重置本地数据         | `just reset-data` 或 `just reset-dev`            | `./scripts/reset-local-data.sh --yes`             | 本地状态混乱需要回到干净态                     |
