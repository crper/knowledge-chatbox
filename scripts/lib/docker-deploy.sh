...
SCRIPT_LABEL="docker-deploy"
# shellcheck disable=SC1091
source "$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)/common.sh"

ROOT_DIR="${REPO_ROOT}"
COMPOSE_FILE="$(resolve_fs_path "${ROOT_DIR}" "${COMPOSE_FILE:-${ROOT_DIR}/docker-compose.yml}")"
ENV_FILE="$(resolve_fs_path "${ROOT_DIR}" "${ENV_FILE:-${ROOT_DIR}/.env}")"
...
validate_env() {
  require_non_empty WEB_PORT API_PORT JWT_SECRET_KEY INITIAL_ADMIN_USERNAME
  validate_port WEB_PORT
  validate_port API_PORT
  if [[ -n "${VITE_API_BASE_URL:-}" && "${VITE_API_BASE_URL}" != "/api" ]]; then
