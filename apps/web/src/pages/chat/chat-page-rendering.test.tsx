...
describe("ChatPage rendering", () => {
  ...
  it("shows the streaming footer hint at the bottom of the session output while submitting", async () => {
    mockWorkspaceState.current = {
      ...mockWorkspaceState.current,
      activeSession: { id: 1, title: "Session A" },
      activeSessionId: 1,
    ...
    const footerHint = await screen.findByTestId("chat-streaming-footer-hint");

    expect(footerHint).toHaveTextContent("正在生成，可随时停止");
