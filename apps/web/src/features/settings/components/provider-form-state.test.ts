...
describe("provider-form-state validation", () => {
  it("returns structured base-url and timeout errors for the active provider", () => {
    ...
    expect(result).toMatchObject({
      fields: {
        primaryBaseUrl: "settings:baseUrlInvalidError",
        providerTimeoutSeconds: "settings:providerTimeoutInvalidError",
      },
  ...
  it("allows an empty OpenAI base url but requires one for Ollama", () => {
    ...
    expect(
      ...
      fields: {
        primaryBaseUrl: "settings:providerTestOllamaBaseUrlMissing",
      },
    });
  });
  ...
  it("requires retrieval embedding model only when override is enabled", () => {
    ...
    expect(
      ...
      fields: {
        retrievalEmbeddingModel: "settings:retrievalEmbeddingModelRequiredError",
      },
    });
  });
